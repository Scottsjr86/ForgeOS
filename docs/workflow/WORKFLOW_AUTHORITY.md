...
ACTIVE_QUESTION=Can ForgeOS manage one explicitly ordered service plan with stable session identity, dependency-gated startup, explicit readiness, bounded startup restart, reverse-order shutdown, typed failure, and no timing or process-name guessing?
...
RETURN_PATH=CLOSE_ONLY_FORGEOS-V1-SESSION-100_THEN_REEVALUATE_CURRENT_AVAILABLE_FRONTIER
