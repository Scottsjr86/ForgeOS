...
BATON_OWNER=FORGEOS-V1-SESSION-201
...
ACTIVE_QUESTION=Can ForgeOS supervise exactly one external Nyx process, use the Nyx-owned public health contract, restart within policy, and stop cleanly on logout?
FIRST_BLOCKER=FORGEOS_HAS_DECLARATIVE_SERVICE_STATE_AND_A_NYX_PUBLIC_HEALTH_CLIENT_BUT_NO_SINGLE_PRODUCT_RUNTIME_THAT_OWNS_ONE_EXTERNAL_NYX_PROCESS_REJECTS_DUPLICATES_APPLIES_BOUNDED_RESTART_AND_STOPS_ON_LOGOUT
ACTIVE_SLICE=FORGEOS-V1-SESSION-201-SLICE-001
SOURCE_IMPLEMENTATION_ALLOWED=YES_EXTERNAL_NYX_PROCESS_SUPERVISION_ONLY
CURRENT_RESULT=SESSION_BOOTSTRAP_CLOSED_MANAGED_SERVICE_LIFECYCLE_ACTIVE
RETURN_PATH=RUN_BEHAVIOR_ONLY_CI_THEN_CLOSE_ONLY_FORGEOS-V1-SESSION-201_IF_GREEN
...
Git inspection, safe Git mutation, version-bound verification, and dedicated
display-manager session bootstrap are closed. `FORGEOS-V1-SESSION-201` is the only
active source skill. Nyx checkpoint transport and remote-agent records remain
available but inactive. The active skill may only supervise one external Nyx process,
consume Nyx-owned public health, reject duplicate instances, apply bounded restart,
and stop cleanly on logout. It may not embed Nyx behavior, own model or conversation
state, call providers, render Forge World, or close distribution work.
