...
ACTIVE_QUESTION=Can ForgeOS inspect the real normal and build dependency graph reachable from forge-core and fail closed on every package outside the explicitly reviewed pure graph, including transitive adapter smuggling?
...
RETURN_PATH=CLOSE_ONLY_FORGEOS-V1-GUARD-001_THEN_REEVALUATE_CONTRACT-000_AND_DIRECT_UNLOCKS
```
