...
ACTIVE_QUESTION=Can ForgeOS receive one exact patch envelope, verify its identity, declared base revision, file table, text-only path-safe payload, and current file preconditions, then apply it without partial acceptance or hidden worktree damage?
...
RETURN_PATH=CLOSE_ONLY_FORGEOS-V1-PATCH-100_THEN_REEVALUATE_GIT-201_AND_CURRENT_AVAILABLE_FRONTIER
