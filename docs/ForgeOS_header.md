...
with typed input routing, and workspace snapshot/crash-journal recovery are closed.
Repository file tree and exact text search are the active V1 capability:

```text
PROGRAM_MODE=SLICE
ACTIVE_QUESTION_CLASS=V1_CAPABILITY
ACTIVE_RELEASE_TARGET=FORGEOS_V1_FIRST_ARMOR
ACTIVE_RELEASE_GATE=FORGEOS-V1-FILE-200
ACTIVE_V1_CONTRIBUTION=REPOSITORY_FILE_TREE_AND_SEARCH
ACTIVE_CAPABILITY_ID=FORGEOS-V1-FILE-200
QUESTION=Can ForgeOS deterministically browse only manifest-approved repository roots, open exact files, and find text without following symlinks, crossing mounts, escaping the repository boundary, or mutating source?
...
OPERATOR_VALIDATION_STATE=PENDING_FOR_FORGEOS-V1-FILE-200
NEXT_REQUIRED_ACTION=EXECUTE_FORGEOS-V1-FILE-200-SLICE-001_AND_RUN_BEHAVIOR_ONLY_CI
...
`FORGEOS-V1-FILE-200` is the only active source skill.
...
This slice may establish only manifest-approved repository tree projection, exact
file opening through FILE-100, deterministic exact-text search, explicit rejected or
unreadable-entry reporting, and editor-buffer composition of a selected file. It may
not save buffers, mutate files, execute commands, inspect Git, call Nyx, add ignore
semantics, or implement the rendered Forge World file panel.
