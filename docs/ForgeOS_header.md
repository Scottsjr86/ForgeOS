...
mutation with isolated worktree control, and version-bound build/test evidence are
closed. Dedicated display-manager session bootstrap is the active V1 capability:

```text
PROGRAM_MODE=SLICE
ACTIVE_QUESTION_CLASS=V1_CAPABILITY
ACTIVE_RELEASE_TARGET=FORGEOS_V1_FIRST_ARMOR
ACTIVE_RELEASE_GATE=FORGEOS-V1-SESSION-200
ACTIVE_V1_CONTRIBUTION=DEDICATED_FORGEOS_SESSION_BOOTSTRAP
ACTIVE_CAPABILITY_ID=FORGEOS-V1-SESSION-200
QUESTION=Can a display manager launch the installed ForgeOS composition root from one deterministic shell-free environment and receive the real startup result?
...
OPERATOR_VALIDATION_STATE=PENDING_FOR_FORGEOS-V1-SESSION-200
NEXT_REQUIRED_ACTION=EXECUTE_FORGEOS-V1-SESSION-200-SLICE-001
