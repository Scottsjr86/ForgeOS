...
dedicated display-manager session bootstrap are closed. Managed ForgeOS and Nyx
service lifecycle is the active V1 capability:

```text
PROGRAM_MODE=SLICE
ACTIVE_QUESTION_CLASS=V1_CAPABILITY
ACTIVE_RELEASE_TARGET=FORGEOS_V1_FIRST_ARMOR
ACTIVE_RELEASE_GATE=FORGEOS-V1-SESSION-201
ACTIVE_V1_CONTRIBUTION=MANAGED_FORGEOS_AND_NYX_SERVICE_LIFECYCLE
ACTIVE_CAPABILITY_ID=FORGEOS-V1-SESSION-201
QUESTION=Can ForgeOS own exactly one external Nyx process, derive readiness only from the Nyx public contract, restart within policy, and stop it cleanly without affecting local development workflows?
...
OPERATOR_VALIDATION_STATE=PENDING_FOR_FORGEOS-V1-SESSION-201
NEXT_REQUIRED_ACTION=EXECUTE_FORGEOS-V1-SESSION-201-SLICE-001
...
`FORGEOS-V1-GIT-201` is the only active source skill.
