...
with exact output history and consistency-checked project-bound native Git inspection are closed. Safe project-bound Git mutation and isolated worktree control are the active V1 capability:

```text
PROGRAM_MODE=SLICE
ACTIVE_QUESTION_CLASS=V1_CAPABILITY
ACTIVE_RELEASE_TARGET=FORGEOS_V1_FIRST_ARMOR
ACTIVE_RELEASE_GATE=FORGEOS-V1-GIT-201
ACTIVE_V1_CONTRIBUTION=PROJECT_BOUND_SAFE_GIT_MUTATION_AND_ISOLATED_WORKTREE_CONTROL
ACTIVE_CAPABILITY_ID=FORGEOS-V1-GIT-201
QUESTION=Can ForgeOS stage, unstage, commit, restore, create, and remove worktrees only from one exact accepted project Git snapshot while preserving unselected files, branches, and worktrees?
...
OPERATOR_VALIDATION_STATE=PENDING_FOR_FORGEOS-V1-GIT-201
NEXT_REQUIRED_ACTION=EXECUTE_FORGEOS-V1-GIT-201-SLICE-001
