...
Nyx health/versioned public-client protocol, and source-backed Forge World projection
with typed input routing are closed. Workspace snapshot and crash-journal recovery is
the active V1 capability:

```text
PROGRAM_MODE=SLICE
ACTIVE_QUESTION_CLASS=V1_CAPABILITY
ACTIVE_RELEASE_TARGET=FORGEOS_V1_FIRST_ARMOR
ACTIVE_RELEASE_GATE=FORGEOS-V1-RECOVERY-100
ACTIVE_V1_CONTRIBUTION=WORKSPACE_SNAPSHOT_AND_CRASH_JOURNAL_PRIMITIVE
ACTIVE_CAPABILITY_ID=FORGEOS-V1-RECOVERY-100
QUESTION=Can ForgeOS atomically retain a versioned safe workspace image while exposing interrupted actions and historical process residue without replaying effects or claiming stale processes are alive?
...
OPERATOR_VALIDATION_STATE=PENDING_FOR_FORGEOS-V1-RECOVERY-100
NEXT_REQUIRED_ACTION=EXECUTE_FORGEOS-V1-RECOVERY-100-SLICE-001_AND_RUN_BEHAVIOR_ONLY_CI
...
`FORGEOS-V1-RECOVERY-100` is the only active source skill.
...
This slice may establish only canonical recovery-image bytes, conservative
interrupted-action and historical-process evidence, explicit recovery assessment,
and generation-guarded atomic publication. Integrated workspace restoration,
service restart, terminal recovery, Nyx reconnection, and recovery UI remain
