...
>
> Source basis: `Forge_OS_V1_base_28.tar`, the canonical V1 skill tree, and the accepted behavior-only CI result through `FORGEOS-V1-GIT-100`.
> `FORGEOS-V1-GIT-101` is currently active and awaiting operator validation.
...
**Raw closed-node count:** 19 of 67, or 28.4%. This is not a release-completion percentage because higher-tier nodes integrate many lower-tier capabilities.
...
| 1 | Local mechanisms | 16 | closed 9, active 1, available 4, locked 2 |
...
- **Direct prerequisites:** `GIT-100` and `CONTRACT-000` ✅
- **Immediate unlock after closure:** None independently; `GIT-201` still requires `GIT-200` and `PATCH-100`.
...
| 🔨 ACTIVE | `FORGEOS-V1-GIT-101` | Git mutation and worktree primitives | ✅ `FORGEOS-V1-GIT-100`<br>✅ `FORGEOS-V1-CONTRACT-000` |
