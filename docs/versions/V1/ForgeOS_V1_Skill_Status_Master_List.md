...
>
> Source basis: `Forge_OS_V1_base_30.tar`, the canonical V1 skill tree, and the accepted behavior-only CI result through `FORGEOS-V1-GIT-101`.
> `FORGEOS-V1-PATCH-100` is currently active and awaiting operator validation.
...
**Raw closed-node count:** 20 of 67, or 29.9%. This is not a release-completion percentage because higher-tier nodes integrate many lower-tier capabilities.
...
| 1 | Local mechanisms | 16 | closed 10, active 1, available 3, locked 2 |
...
- **Direct prerequisites:** `PATH-000`, `STATE-000`, and `HASH-000` ✅
- **Immediate unlock after closure:** None independently; `GIT-201` still requires `GIT-200`, but its `GIT-101` and `PATCH-100` prerequisites will be complete.
...
| 🔨 ACTIVE | `FORGEOS-V1-PATCH-100` | Patch identity, base validation, and safe application primitive | ✅ `FORGEOS-V1-PATH-000`<br>✅ `FORGEOS-V1-STATE-000`<br>✅ `FORGEOS-V1-HASH-000` |
